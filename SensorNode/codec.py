# Description: The codec is intended to decode and encode between bytes and lists of packets.
# The binaryPacket, E4E_Data_IMU, E4E_Data_Audio_raw8, E4E_Data_Audio_raw16, E4E_Data_Raw_File_CTS,...
# ...E4E_Data_Raw_File_ACK, E4E_Data_Raw_File, and binaryPacketParser classes are directly copied
# from https://github.com/UCSD-E4E/e4e-tools/blob/binaryProtocol/binary_protocol/Binary%20Protocol%20Decoder.ipynb
...
class binaryPacket:
    ...
    def __init__(self, payload: bytes, packetClass: int, packetID: int,
                 sourceUUID: uuid.UUID, destUUID: uuid.UUID):
    ...
    def __str__(self):
        ...
        length = 4
        return '%s' % ' '.join(string[i:i + length]
    ...
    def from_bytes(cls, packet: bytes):
        ...
        srcUUID = uuid.UUID(bytes=srcUUID)
        destUUID = uuid.UUID(bytes=destUUID)
        return binaryPacket(payload, pcls, pid, srcUUID, destUUID)
...
class E4E_Data_IMU(binaryPacket):
    ...
    def __init__(self, src: uuid.UUID, dest: uuid.UUID, accX, accY, accZ,
...
class E4E_Data_Audio_raw8(binaryPacket):
    ...
    def from_bytes(cls, packet: bytes):
        ...
        srcUUID = uuid.UUID(bytes=srcUUID)
        destUUID = uuid.UUID(bytes=destUUID)
        return E4E_Data_Audio_raw8(audioData, srcUUID, destUUID, timestamp)
...
class E4E_Data_Audio_raw16(binaryPacket):
    ...
    def from_bytes(cls, packet: bytes):
        ...
        srcUUID = uuid.UUID(bytes=srcUUID)
        destUUID = uuid.UUID(bytes=destUUID)
        return E4E_Data_Audio_raw16(audioData, srcUUID, destUUID, timestamp)
...
class E4E_Data_Raw_File_CTS(binaryPacket):
    ...
    def from_bytes(cls, packet: bytes):
        ...
        srcUUID = uuid.UUID(bytes=srcUUID)
        destUUID = uuid.UUID(bytes=destUUID)
        return E4E_Data_Raw_File_CTS(fileID, ack, srcUUID, destUUID)
...
class E4E_Data_Raw_File_ACK(binaryPacket):
    ...
    def from_bytes(cls, packet: bytes):
        ...
        srcUUID = uuid.UUID(bytes=srcUUID)
        destUUID = uuid.UUID(bytes=destUUID)
        return E4E_Data_Raw_File_ACK(fileID, seq, ack, srcUUID, destUUID)
...
class E4E_Data_Raw_File(binaryPacket):
    ...
    def from_bytes(cls, packet: bytes):
        ...
        srcUUID = uuid.UUID(bytes=srcUUID)
        destUUID = uuid.UUID(bytes=destUUID)
        return E4E_Data_Raw_File(fileID, seq, blob, srcUUID, destUUID)
